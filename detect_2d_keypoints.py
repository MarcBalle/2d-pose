...
def parse_arguments():
    ...
    parser.add_argument('--config_det', '-cd', type=str, required=True, help='mmdetection model config file')
...
if __name__ == '__main__':
    ...
    inferencer = MMPoseInferencer(pose2d=args.config_pose, det_model=args.config_det, det_cat_ids=[0], device=device)  # the category id of 'human' class is [0]
    ...
    out = cv2.VideoWriter(os.path.join(args.output, video_name + '.2dkeypoints' + video_ext), cv2.VideoWriter_fourcc(*'DIVX'), fps, size)
    out.write(bb_img)
