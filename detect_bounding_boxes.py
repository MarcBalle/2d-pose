...
def parse_arguments():
    ...
    parser.add_argument(
        "--frames", "-f", type=str, default="", help="Number of frames to analyze"
    )
    parser.add_argument(
        "--config", "-c", type=str, required=True, help="mmdetection model config file"
    )
    parser.add_argument(
...
if __name__ == "__main__":
    ...
    size = (w, h)
    out = cv2.VideoWriter(
        os.path.join(args.output, "bb_output.avi"),
